...
import com.baiye.yudada.mapper.ScoringResultMapper;
import com.baiye.yudada.model.dto.scoringResult.ScoringResultQueryRequest;
import com.baiye.yudada.model.entity.ScoringResult;
import com.baiye.yudada.model.entity.User;
import com.baiye.yudada.model.vo.ScoringResultVO;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(resultScoreRange == null || resultScoreRange <= 0, ErrorCode.PARAMS_ERROR, "结果分数范围必须大于0");
        ...
        if (StringUtils.isNotBlank(resultDesc)) {
            ThrowUtils.throwIf(resultDesc.length() > 256, ErrorCode.PARAMS_ERROR, "结果描述不能超过 256");
        }
        if (StringUtils.isNotBlank(resultProp)) {
            ThrowUtils.throwIf(resultProp.length() > 128, ErrorCode.PARAMS_ERROR, "结果属性不能超过 128");
        }
        // 补充校验规则
        if (appId != null) {
